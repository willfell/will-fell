...
const PortfolioCard: FC<PortfolioCardProps> = memo(({ item, onClick, isCompact = false, hideDescription = false }) => {
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
        ...
        {!hideDescription && (isCompact || location) && (
          <div className="flex flex-wrap gap-2 mb-2">
            {location && (
              <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">{location}</span>
            )}
            {date && (
              <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">{date}</span>
            )}
          </div>
        )}
        ...
        {isCompact && techStack && techStack.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            ...
            {techStack.length > 3 && (
              <span className="text-xs text-white/70">+{techStack.length - 3} more</span>
            )}
          </div>
        )}
