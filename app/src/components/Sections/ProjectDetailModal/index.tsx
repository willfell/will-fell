...
const ProjectDetailModal: FC<ProjectDetailModalProps> = memo(({ project, onClose }) => {
  ...
  useEffect(() => {
    ...
    const modalContent = modalRef.current?.querySelectorAll('.animate-on-scroll');
            ...
            <div className="max-w-6xl mx-auto">
              ...
              {!isPassionProject && location && (
                <div className="flex gap-2 items-center mb-4">
                  <span className="bg-white/20 text-white px-3 py-1 rounded-full text-sm">{location}</span>
                  {date && <span className="bg-white/20 text-white px-3 py-1 rounded-full text-sm">{date}</span>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-6">
            ...
            <div className="md:col-span-1 space-y-6 animate-fadeIn">
              {why ? (
                <div className="bg-sage-green/10 p-5 rounded-lg shadow-sm">
                  <h3 className="text-xl font-semibold text-forest-green mb-3">Why I Built This</h3>
                  <p className="text-stone-700">{why}</p>
                </div>
              ) : summary ? (
                <div className="bg-sage-green/10 p-5 rounded-lg shadow-sm">
                  <h3 className="text-xl font-semibold text-forest-green mb-3">Summary</h3>
              ...
              {githubUrl && (
                <div className="bg-sage-green/10 p-5 rounded-lg shadow-sm">
                  <h3 className="text-xl font-semibold text-forest-green mb-3">GitHub</h3>
                  ...
                  {isLoading ? (
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-semibold">{githubData.stargazers_count}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-stone-700">Watchers</span>
                        <span className="font-semibold">{githubData.watchers_count}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-stone-700">Forks</span>
                        <span className="font-semibold">{githubData.forks_count}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-stone-700">Last Updated</span>
                        <span className="font-semibold">{new Date(githubData.updated_at).toLocaleDateString()}</span>
                      </div>
                      <div className="pt-2">
                        <a
                          href={githubUrl}
            ...
            <div className="md:col-span-1 animate-fadeIn">
              {how && (
                <div className="bg-sage-green/10 p-5 rounded-lg h-full shadow-sm">
                  <h3 className="text-xl font-semibold text-forest-green mb-3">How It Works</h3>
            ...
            <div className="md:col-span-1 animate-fadeIn">
              {techStack && techStack.length > 0 && (
                <div className="bg-sage-green/10 p-5 rounded-lg shadow-sm">
                  <h3 className="text-xl font-semibold text-forest-green mb-3">Tech Stack</h3>
